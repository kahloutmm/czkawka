...
fn add_chosen_directories(window_main: &Window, tree_view: &TreeView, excluded_items: bool) {
    ...
    let file_chooser = gtk4::FileChooserDialog::builder().title(&folders_to).action(gtk4::FileChooserAction::SelectFolder).transient_for(window_main).modal(true).build();
